...
class ResNet(nn.Module):
    def __init__(self, model, in_channels=3, output_stride=16):
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, model, config, in_channels=3,
                output_stride=16, freeze_bn=False, freeze_backbone=False, **_):
                
        super(DeepLab, self).__init__()
        # assert ('xception' or 'resnet' in backbone)
        if 'resnet' in config.backbone:
            self.backbone = ResNet(model, in_channels=in_channels, output_stride=output_stride)
        ...
        self.decoder = Decoder(low_level_channels, num_classes)###xception low level channel=256, resnet=128
