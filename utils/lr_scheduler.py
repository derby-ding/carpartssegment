...
class Poly(_LRScheduler):
    ...
    def get_lr(self):
        ...
        factor = max(pow((1 - 1.0 * T / self.N), 0.9), 0.02)####最小值为原值的0.02倍
...
class OneCycle(_LRScheduler):
    def __init__(self, optimizer, num_epochs, iters_per_epoch=0, last_epoch=-1,
        ...
        self.final_lrs = [opt_grp['lr']/(div_factor * 1) for opt_grp in optimizer.param_groups]###factor*1e4
...
if __name__ == "__main__":
    ...
    # from utils.lr_scheduler import OneCycle
    ...
    lrs = []
    mementums = []
    # onec = OneCycle(optimizer, epochs, iters_per_epoch=100)
    lr_scheduler = OneCycle(optimizer, epochs, iters_per_epoch=100, div_factor=10, momentums=(0.95, 0.97), phase1=0.1)
